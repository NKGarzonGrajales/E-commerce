...
const CartView: React.FC<{ token: any; userData: IUserSession }> = ({ token }) => {
    const router = useRouter();
    
    // Conectamos directamente con el store global de Zustand
    ...
    // Cálculo dinámico basado en el estado global
    const totalBeforeTax = cart.reduce(
        (acc, item) => acc + item.price * (item.quantity || 1),
        0
    );
    
    const totalC = totalBeforeTax > 0 ? totalBeforeTax + tax + shipping : 0;
                ...
                <div className="w-full sm:w-3/4 bg-white px-10 py-10">
                    ...
                    <div className="md:flex items-stretch py-8 md:py-10 lg:py-8 max-h-[360px] flex-col mb-4 overflow-y-scroll p-4">
                        {cart.length ? (
                            ...
                            <p className="text-center text-gray-500 py-6">Your cart awaits its first item!</p>
                        )}
                    </div>
                </div>
                    ...
                    <ul>
                        {cart.map((item) => (
                            <li key={item.id} className="flex justify-between items-center border-b py-2">
                                ...
                                <p className="text-md font-semibold">${item.price * (item.quantity || 1)}</p>
                            </li>
                        ))}
                    </ul>
                    <div className="mt-4">
                        ...
                        <div className="flex justify-between mb-2">
                            <span>Taxes</span>
                            <span>${cart.length > 0 ? "1.99" : "0.00"}</span>
                        </div>
                        <div className="flex justify-between mb-2">
                            <span>Shipping</span>
                            <span>Freebie!</span>
                        ...
                        <button
                            ...
                            className="bg-gray-950 text-white text-xl font-bold py-2 px-4 rounded-lg mt-4 w-full hover:bg-green-950 hover:opacity-95 transition-colors"
                        >
                            Checkout
                        </button>
                    </div>
